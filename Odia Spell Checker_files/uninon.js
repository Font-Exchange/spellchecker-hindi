...
function initApp()
{
	...
	document.getElementById("wordCount").innerHTML="Akruti : "+wordListAkruti.length+", Unicode : "+wordListUnicode.length;
	if(typeof downloadFileName!="undefined" && mode!="mini")
	{
		document.getElementById("downloadLink").innerHTML='&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="'+downloadFileName+'">Download : '+downloadFileName+'</a>';
	}
	else
	{
		document.getElementById("downloadLink").innerHTML='&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="http://www.odiasepellcheck.com">Visit for Updated Spell Cheker</a>';
	}
	initLanguage();
	initDatabase();
}
...
function removePriviusWordList()
{
	...
	{
		document.body.removeChild(wordListObjParent);
...
function displayWordList(word,obj)
{
	...
	document.body.appendChild(wordListObjParent);
...
{
	callAjax("insertword.htm?word="+word+"&lang="+document.getElementById("language").value,"",
	function(resp)
	{
		alert(resp);
		removePriviusWordList();
...
function addEditMenu(word)
{
	...
	obj.innerHTML='<input type="button" value="Ignore" onclick="updateExitText()"><input type="text" style="width:57px;" id="editWord" value="'+word+'" /><input type="button" value="Change" background-color: #008CBA; onclick="updateExitText()" />';
...
function checkSpell()
{
	...
	obj.innerHTML=out;
	removeWaitScreen();
	document.getElementById("checkSpellButton").disabled=true;
...
function initLanguage()
{
		if(lang=="unicode")
		{
			...
			document.getElementById("editor").className = "panel-body context";
		}
		else 
		{
			wordList = wordListAkruti;
			wordConvrtion = wordConvrtionAkruti;
			document.getElementById("editor").className = "editorAkruti";
		}
}
...
function pasteText()
{
	//setTimeout("removeTextFormating()",1);
